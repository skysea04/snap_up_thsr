...
class PageParser:
    ...
    @staticmethod
    def get_booking_method_radio(
        booking_page: Tag, booking_request: BookingRequest
    ) -> Dict:
        if booking_request.booking_method == BookingMethod.TIME:
            return booking_page.find(**BookingPage.BOOKING_METHOD_TIME_RADIO).get(
                "value"
            )

        return booking_page.find(**BookingPage.BOOKING_METHOD_TRAIN_NO_RADIO).get(
            "value"
        )
    ...
    def get_train_lst(
        ...
        select_train_page: Tag,
        buy_discount_ticket: bool,
    ) -> List[Train]:
        train_lst = []
        train_elem_lst: List[Tag] = select_train_page.find_all(
            "label", **SelectTrainPage.TRAIN_LIST
        )

        for train_elem in train_elem_lst:
            str_departure_time = train_elem.find("input").get(
                SelectTrainPage.DEPARTURE_TIME
            )
            if str_departure_time < AvailableTime.get_label_name(
                booking_request.earliest_depart_time
            ):
                continue
            ...
            if str_arrival_time > AvailableTime.get_label_name(
                booking_request.latest_arrival_time
            ):
                break

            discount_elems: List[Tag] = train_elem.find(
                **SelectTrainPage.DISCOUNT_LIST
            ).find_all("span")
            ...
            dt_travel_time = dt.strptime(
                train_elem.find("input").get(SelectTrainPage.TRAVEL_TIME), "%H:%M"
            )
    ...
    def get_not_member_radio(confirm_ticket_page: Tag):
        return confirm_ticket_page.find(**ConfirmTicketPage.NOT_MEMBER).get(
            ConfirmTicketPage.FORM_VALUE
        )

    @staticmethod
    def get_tgo_member_radio(confirm_ticket_page: Tag):
        return confirm_ticket_page.find(**ConfirmTicketPage.TGO_MEMBER).get(
            ConfirmTicketPage.FORM_VALUE
        )
    ...
    def get_booked_ticket_info(page: Tag) -> THSRTicket:
        ...
        payment_deadline = (
            page.find(**CompleteBookingPage.PAYMENT_DEADLINE).find_next().text
        )
        ...
        depart_station_text = (
            page.find(**CompleteBookingPage.TICKET_DEPARTURE_STATION).find_next().text
        )
        arrival_station_text = (
            page.find(**CompleteBookingPage.TICKET_ARRIVAL_STATION).find_next().text
        )
        ...
        depart_station = next(
            filter(lambda s: s[1] == depart_station_text, Station.choices)
        )[0]
        arrival_station = next(
            filter(lambda s: s[1] == arrival_station_text, Station.choices)
        )[0]
...
class BookingProcessor:
    @staticmethod
    def submit_booking_condition(
        sess: THSRSession, booking_page: Tag, booking_form: BookingForm
    ) -> Tag:
        ...
        while not ok:
            if err_msg == ErrorPage.ERROR_SECURITY_CODE and err_cnt < 3:
                ...
                booking_form.security_code = (
                    PageParser.get_security_code(sess, booking_page),
                )
                next_page = sess.submit_booking_condition(
                    PageParser.get_next_page_path(BookingPage, booking_page),
                    booking_form.model_dump(by_alias=True),
